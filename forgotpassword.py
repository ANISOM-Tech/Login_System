...
class ForgotPassword:
    def __init__(self, root1):
        ...
        Label(self.root, text="User Details", bg="#D4F1F4", fg="#000C66",
              font=("Goudy old style", 20, "bold", "underline")).place(x=120, y=10)
        # title: First Name
        ...
        # Action buttons
        # log in button
        Button(self.root, text="Check Password", bg="white", fg="#000C66", cursor="hand2", command=self.fg_pass,
    ...
    def fg_pass(self):
        ...
        if self.name.get() == "" or self.surname.get() == "" or self.email.get() == "" or self.contact.get() == "" or \
            ...
            result, username1, password1 = pr.ck_pass(self.name.get(), self.surname.get(), self.email.get(),
                                                      self.contact.get())
            print(result)
            if result == 0:
                tkinter.messagebox.showerror("Error", "You have entered wrong details.", parent=self.root)
            else:
                tkinter.messagebox.showinfo("Password fetched",
                                            f"We got your username and password \n Username : {username1}"
                                            f" \n Password : {password1}",
                                            parent=self.root)
                self.root.quit()
